...
import { TextField } from 'uniforms-ionic';
...
test('<TextField> - renders an input with correct type', () => {
  ...
  expect(wrapper.find(IonInput)).toHaveLength(1);
  expect(wrapper.find(IonInput).prop('type')).toBe('text');
});
...
test('<TextField> - renders an input with correct value (model)', () => {
  ...
  expect(wrapper.find(IonInput)).toHaveLength(1);
  expect(wrapper.find(IonInput).prop('value')).toBe('y');
});
...
test('<TextField> - renders an input which correctly reacts on change', () => {
  ...
  expect(wrapper.find(IonInput)).toHaveLength(1);
  // @ts-ignore
  wrapper.find(IonInput).prop('onIonChange')({ target: { value: 'y' } });
